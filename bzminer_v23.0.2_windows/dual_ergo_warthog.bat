...
:: use --i2 to adjust the ratio of wart sha hashes per ergo hashes. the higher means more sha hashes, which can reduce ergo hashes. 0 is auto tune, and will attempt to find the highest number of both ergo and wart hashes, which can drastically reduce ergo hashes. the more gpus dual mining, the less sha hashes you may need to do on each hashrate to keep the cpu verus hashrate full for warthog

bzminer -a ergo -w 0000 -p us.ergo.herominers.com:1180 --a2 warthog --w2 1111 --p2 stratum+tcp://pool.us.woolypooly.com:3140 --i2 0 --nc 1 --cpu_threads 0 --cpu_threads_start_offset 0 --warthog_max_ram_gb 0 --warthog_verus_hr_target 0
